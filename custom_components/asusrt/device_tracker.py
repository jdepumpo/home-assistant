...
Modified due to incompability with AsusWRT Component
...
PLATFORM_SCHEMA = vol.All(
    ...
    PLATFORM_SCHEMA.extend({
        ...
        vol.Optional(CONF_PROTOCOL, default='telnet'): vol.In(['ssh', 'telnet']),
...
def get_scanner(hass, config):
    ...
    scanner = AsusRtDeviceScanner(config[DOMAIN])
...
class AsusRtDeviceScanner(DeviceScanner):
    ...
    def _get_wl(self):
        ...
        if not lines:
            return {}
        result = self._parse_lines(lines, _WL_REGEX)
        devices = {}
    ...
    def _get_leases(self, cur_devices):
        ...
        result = self._parse_lines(lines, _LEASES_REGEX)
        devices = {}
        for device in result:
    ...
    def _get_neigh(self, cur_devices):
        ...
        if not lines:
            return {}
        result = self._parse_lines(lines, _IP_NEIGH_REGEX)
        devices = {}
        for device in result:
    ...
    def _get_arp(self):
        ...
        if not lines:
            return {}
        result = self._parse_lines(lines, _ARP_REGEX)
        devices = {}
        for device in result:
            if device['mac'] is not None:
                mac = device['mac'].upper()
    ...
    def _parse_lines(self, lines, regex):
...
class SshConnection(_Connection):
    ...
    def run_command(self, command):
        ...
        try:
            ...
            _LOGGER.error("run_command -> Unexpected error: %s", sys.exc_info()[0])
            self.disconnect()
            return None
    ...
    def connect(self):
        ...
        _LOGGER.debug("Asusrt connected")
    ...
    def disconnect(self):   \
        ...
        try:
            ...
            self._ssh = None
        _LOGGER.debug("Asusrt disconnected")
